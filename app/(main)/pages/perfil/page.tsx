...
const Perfil = () => {
    ...
    const savePerfil = () => {
        setSubmitted(true);
//se perfil nao tem ID, cria um novo
        if(!perfil.id){
...
//se ja tiver um ID Edita o perfil
        }else {
        ...
        if(perfil.id){
            ...
            .then((response)=>{
                ...
                toast.current?.show({
                    severity:"error",
                    summary: "Erro!",
                    detail: "Erro ao excluir o perfil!" + error.data.message
                });
            })
        }
    };
    ...
    const deleteSelectedPerfis = () => {
        Promise.all(selectedPerfis.map(async (_perfil) =>{
            ...
            toast.current?.show({
                severity:"error",
                summary: "Erro!",
                detail: "Erro ao excluir os perfis!" + error.data.message
            });
        })
    };
                ...
                <div className="card">
                    ...
                    <Dialog visible={deletePerfilDialog} style={{ width: '450px' }} header="Confirm" modal footer={deletePerfilDialogFooter} onHide={hideDeletePerfilDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {perfil && (
                                <span>
                                    tem certeza que deseja exluir esse perfil?
                                </span>
                            )}
                        </div>
                    </Dialog>
