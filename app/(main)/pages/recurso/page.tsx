...
const Recurso = () => {
    ...
    const saveRecurso = () => {
        setSubmitted(true);
//se recurso nao tem ID, cria um novo
        if(!recurso.id){
            ...
            .then((response)=>{
                ...
                toast.current?.show({
                    severity:"error",
                    summary: "Erro!",
                    detail: "Erro ao salvar o recurso" 
                });
            })
//se ja tiver um ID Edita o recurso
        }else {
        ...
        if(recurso.id){
            ...
            .then((response)=>{
                ...
                toast.current?.show({
                    severity:"error",
                    summary: "Erro!",
                    detail: "Erro ao excluir o recurso!" + error.data.message
                });
            })
        }
    };
    ...
    const deleteSelectedRecursos = () => {
        Promise.all(selectedRecursos.map(async (_recurso) =>{
            ...
            toast.current?.show({
                severity:"error",
                summary: "Erro!",
                detail: "Erro ao excluir os recursos!" + error.data.message
            });
        })
    };
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    <Dialog visible={recursoDialog} style={{ width: '450px' }} header="Detalhes do recurso" modal className="p-fluid" footer={recursoDialogFooter} onHide={hideDialog}>
                        ...
                        <div className="field">
                            <label htmlFor="login">Chave</label>
                            <InputText
                                id="chave"
                                value={recurso.chave}
                    ...
                    <Dialog visible={deleteRecursoDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteRecursoDialogFooter} onHide={hideDeleteRecursoDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {recurso && (
                                <span>
                                    tem certeza que deseja exluir o recurso <b>{recurso.nome}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
                    ...
                    <Dialog visible={deleteRecursosDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteRecursosDialogFooter} onHide={hideDeleteRecursosDialog}>
                        <div className="flex align-items-center justify-content-center">
                            <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                            {recurso && <span>Tem certeza que deseja deletar os recursos selecionados?</span>}
                        </div>
                    </Dialog>
                </div>
            </div>
